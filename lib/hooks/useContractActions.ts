...
export function useOptInToAsset() {
  ...
  return useMutation({
    mutationFn: async ({ assetId }: { assetId: number }) => {
      console.log('DEBUG: useOptInToAsset initiated', { assetId, activeAddress })
      if (!activeAddress || !transactionSigner) throw new Error('Wallet not connected')
    ...
    onSuccess: async (_, { assetId }) => {
      console.log('DEBUG: useOptInToAsset - onSuccess - refetching queries')
      await qc.refetchQueries({ queryKey: ['asset-balance', activeAddress, assetId], type: 'active' })
    },
    onError: (e) => {
      console.error('DEBUG: useOptInToAsset - mutation error', e)
    }
  })
}